...
def listado_dispositivos_medicos_view(request):
    dispositivos_list = ListadoDispositivosMedicos.objects.all().order_by('item_nro')  # Ordenar por item_nro
        ...
        if form.is_valid():
            ...
            for index, row in data.iterrows():
                try:
                    ...
                    error_message = f"Error al procesar la fila {index + 1}: {row}. Error: {str(e)}"
                    print(error_message)
