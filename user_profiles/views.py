...
def assign_unit_view(request):
    ...
    if 'selected_user_id' in request.session:
        ...
        profile, _ = UserProfile.objects.get_or_create(user=selected_user)
        current_unit = profile.unidad_medica
        assign_unit_form = AssignUnitForm(initial={'group': current_group, 'unidad_medica': current_unit})
    else:
    ...
    if 'assign_unit' in request.POST and assign_unit_form:
        ...
        if assign_unit_form.is_valid():
            ...
            messages.success(request, f'El Usuario: {selected_user} ha cambiado de {current_group} a {selected_group}.')
            messages.success(request, f'Y su Unidad Médica ha cambiado de {current_unit} a {selected_unit}.')
